...
import {handleTestGoal} from "./src/testGoal"
    ...
    bot.on("chat", async (username: string, message: string) => {
      ...
      if (message === "blocks") {
        ...
        const blocksStr = Object.entries(visibleBlocksResult.BlockTypes)
          .map(
            ([blockName, { x, y, z }]) => `${blockName} at (${x}, ${y}, ${z})`
          )
          .join(", ");
        ...
        switch (subcommand) {
          ...
          case "pullup": {
            ...
            if (!targetPlayer || !targetPlayer.entity) {
              ...
              bot.chat(
                `Moving to jibbum at (${x.toFixed(1)}, ${y.toFixed(
                  1
                )}, ${z.toFixed(1)})...`
              );
              await navigation.move(x, y, z);
          ...
          case "goal:": {
            await handleTestGoal(bot, functionCaller, parts);
            break;
          }
