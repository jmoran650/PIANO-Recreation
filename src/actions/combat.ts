...
export class CombatService {
  ...
  constructor(bot: Bot, sharedState: SharedAgentState) {
    this.bot = bot;
  ...
  attack(targetName: string) {
    ...
    if (!pvp || typeof pvp.attack !== "function") {
      const errorMsg =
        "[CombatService] pvp plugin or pvp.attack function unavailable.";
      console.error(errorMsg);
      throw new Error(errorMsg);
    }

    const targetIsPlayer =
      this.bot.players[targetName.toLowerCase()] !== undefined;
    console.log(
      `[CombatService] attack: Target="${targetName}", IsPlayer=${targetIsPlayer}`
    );

    const entitiesToSearch = Object.values(this.bot.entities);
    // Filter potential targets AND ensure they have a position
    const potentialTargets = entitiesToSearch.filter(
      (entity): entity is Entity => {
        ...
        if (targetIsPlayer) {
          return (
            entity.type === "player" &&
            entity.username?.toLowerCase() === targetName.toLowerCase()
          );
        } else {
          return (
            entity.type === "mob" &&
    ...
    console.log(
      `[CombatService] attack: Found ${potentialTargets.length} potential target(s) after filtering.`
    );

    if (potentialTargets.length === 0) {
      const message = `[CombatService] No valid ${
        targetIsPlayer ? "player" : "mob"
      } named "${targetName}" found nearby with complete data.`;
      console.log(message);
    ...
    const closestTarget = potentialTargets.reduce((nearest, currentTarget) => {
      try {
        const distToCurrent = this.bot.entity.position.distanceTo(
          currentTarget.position
        );
        const distToNearest = this.bot.entity.position.distanceTo(
          nearest.position
        );
        ...
        console.warn(
          `[CombatService] Error comparing distances during reduce: ${msg}. Skipping comparison.`
        );
        return nearest;
      }
    });
    ...
    console.log(
      `[CombatService] Closest target selected: ID ${
        closestTarget?.id
      }, Type: ${closestTarget?.type}, Name: ${
        closestTarget?.username || closestTarget?.name
      }, IsValid: ${closestTarget?.isValid ?? "N/A"}`
    );

    // Check if a target was selected and if it's considered valid (if the property exists)
    const isTargetValid =
      closestTarget &&
      (closestTarget.isValid !== undefined ? closestTarget.isValid : true);
    ...
    if (!isTargetValid) {
      ...
      throw new Error(
        `Cannot attack "${targetName}": Target entity is invalid or missing.`
      );
    }
    // --- End Refined Check ---
    ...
    console.log(
      `[CombatService] Attacking the nearest ${targetType} "${targetName}" (ID: ${closestTarget.id}) at ${closestTarget.position.toString()}...`
    );
    ...
    try {
      ...
      this.bot.once("stoppedAttacking", () => {
        ...
        if (currentTargetEntity && currentTargetEntity.isValid !== false) {
          console.log(
            `[CombatService] Target ${targetType} "${targetName}" (ID: ${closestTarget.id}) still alive after attack cycle.`
          );
          console.log(
            "[CombatService] Re-initiating attack (recursive call - CAUTION)."
          );
          this.attack(targetName);
        } else {
          console.log(
            `[CombatService] Target ${targetType} "${targetName}" (ID: ${closestTarget.id}) killed, despawned, or invalid after attack.`
          );
        }
      });
    } catch (err: unknown) {
      ...
      console.error(
        `[CombatService] Error during pvp.attack execution on ${targetType} "${targetName}": ${errMsg}`
      );
