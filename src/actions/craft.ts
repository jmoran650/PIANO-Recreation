...
import { Recipe } from 'prismarine-recipe'; // <-- Import Recipe type
...
export class CraftingService {
 ...
 public async craft(goalItem: string): Promise<void> {
  ...
  if (potentiallyNeedsTable) {
   ...
   if (!tableReference) {
    ...
    // FIX: Use Recipe type, resolves no-explicit-any and no-unsafe-member-access
    const trulyNeedsTable = allRecipesCheck.some(
     (r: Recipe) => r.requiresTable
    );
    ...
    console.log(
     // FIX: Use .toString() for Vec3
     `[CraftingService] Secured crafting table at ${tableReference.position.toString()} for potential use.`
    );
   }
  } else {
   console.log(
  ...
  if (!possibleRecipesFor || possibleRecipesFor.length === 0) {
   ...
   if (allRecipesWithTable.length > 0 || allRecipesWithoutTable.length > 0) {
    ...
    if (firstRecipe && firstRecipe.delta) {
     const needed = firstRecipe.delta
      .filter((d: { id: number; count: number }) => d.count < 0) // Filter for required items (negative delta)
      .map(
   ...
   // FIX: Use .toString() for Vec3 and fix restrict-plus-operands
   reason += `${
    tableReference
     ? ' (using table at ' + tableReference.position.toString() + ')'
     : ' (no table used)'
   }.`;
  ...
  try {
   ...
   return;
   // FIX: Change catch type to unknown
  } catch (err: unknown) {
   // FIX: Safely access error message
   const message = err instanceof Error ? err.message : String(err);
   console.error(
    `[CraftingService] Failed crafting "${goalItem}" with recipe: ${message}`
   );
   ...
   // FIX: Use safe message in new Error
   throw new Error(`Failed to craft ${goalItem}: ${message}`);
  }
 }
 ...
 private async findCraftingTable(): Promise<Block | null> {
  ...
  if (tableBlock) {
   console.log(
    // FIX: Use .toString() for Vec3
    `[CraftingService] Found crafting table within interaction range at ${tableBlock.position.toString()}.`
   );
   return tableBlock;
  }
  ...
  if (tableBlock) {
   console.log(
    // FIX: Use .toString() for Vec3
    `[CraftingService] Found crafting table nearby (within ${this.TABLE_SEARCH_RANGE} blocks) at ${tableBlock.position.toString()}. Moving closer...`
   );
   // FIX: Change catch type to unknown
   try {
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.error(
     // FIX: Use .toString() for Vec3 and safe message
     `[CraftingService] Failed to move to nearby table at ${tableBlock.position.toString()}: ${message}`
    );
  ...
  for (const pos of cachedPositions) {
   ...
   if (block && block.name === 'crafting_table') {
    console.log(
     `[CraftingService] Found valid cached crafting table at ${pos.toString()}. Moving closer...`
    );
    // FIX: Change catch type to unknown
    try {
     ...
     console.warn(
      // FIX: Use .toString() for Vec3 and safe message
      `[CraftingService] Found cached table at ${pos.toString()} but failed to navigate: ${message}. Trying next cache entry.`
     );
    ...
    console.log(
     // FIX: Use .toString() for Vec3
     `[CraftingService] Cached position ${pos.toString()} is no longer a crafting table. Skipping.`
    );
  ...
  if (tableItemInInventory) {
   ...
   try {
    ...
    console.log(
     // FIX: Use .toString() for Vec3
     `[CraftingService] Successfully placed table from inventory at ${placedTable.position.toString()}. Moving to interact...`
    );
    ...
    // FIX: Change catch type to unknown
   } catch (placeOrNavErr: unknown) {
    // FIX: Safely access error message
    const message =
     placeOrNavErr instanceof Error
      ? placeOrNavErr.message
      : String(placeOrNavErr);
    console.error(
     `[CraftingService] Failed to place table from inventory or navigate to it: ${message}`
    );
 ...
 private async placeCraftingTable(): Promise<Block> {
  ...
  if (!safePos) {
   throw new Error(
    // FIX: Use .toString() for Vec3
    `[CraftingService] No valid spot found near ${this.bot.entity.position.toString()} to place the crafting table!`
   );
  }
  ...
  if (
   ...
   throw new Error(
    // FIX: Use .toString() for Vec3
    `[CraftingService] Cannot place crafting table at ${safePos.toString()}: ${reason}.`
   );
  }

  console.log(
   // FIX: Use .toString() for Vec3 (x2)
   `[CraftingService] Attempting internal placement of crafting table at ${safePos.toString()} onto block ${
    referenceBlock.name
   } at ${referenceBlock.position.toString()}`
  );
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
   ...
   try {
    ...
    if (placedBlock?.name === 'crafting_table') {
     console.log(
      // FIX: Use .toString() for Vec3
      `[CraftingService] Internal placement: Crafting table placed successfully at ${safePos.toString()}!`
     );
     ...
     console.warn(
      // FIX: Use .toString() for Vec3
      `[CraftingService] Attempt ${attempt}: Placed block at ${safePos.toString()}, but it wasn't a crafting table (found ${
       placedBlock?.name
    ...
    const message = err instanceof Error ? err.message : String(err);
    console.log(
     `[CraftingService] Attempt ${attempt} to internally place crafting table failed: ${message}`
    );
    if (attempt === maxRetries) {
     throw new Error(
 ...
 public async useCraftingTable(): Promise<void> {
  ...
  // FIX: Use .toString() for Vec3
  console.log(`[CraftingService] Activating crafting table at ${pos.toString()}`);
  // FIX: Change catch type to unknown
  try {
