...
function escapeRegExp(str: string): string {
  ...
  async function goal_breakdown(
    ...
    for (const [item, desc] of Object.entries(minecraftItems)) {
      ...
      // Allow an optional trailing "s" and an optional parenthesized number (with optional whitespace).
      const pattern = `\\b${escapedItem}(s)?\\s*(\$begin:math:text$\\\\d+\\$end:math:text$)?\\b`;
    ...
    for (const [block, desc] of Object.entries(minecraftBlocks)) {
      ...
      const pattern = `\\b${escapedBlock}(s)?\\s*(\$begin:math:text$\\\\d+\\$end:math:text$)?\\b`;
      const regex = new RegExp(pattern, "i");
      if (regex.test(step)) {
    ...
    const steps = response
      ...
      .map((s) => s.trim())
      .filter((s) => s.length > 0);
    return { steps, debugPrompt: prompt };
  }
  ...
  async function checkFunctionCallDetailed(
    ...
    const promptUsed = getGoalToFuncCallPrompt(
        step, 
        //inventory
   ...
   *
   * This version now extracts a valid function call substring using a regex and then
   * checks using an "includes" condition.
   */
  function updateInventoryFromFuncCall(
    ...
    // Helper to parse inside the parentheses.
  ...
  // To avoid repetition inside updateInventoryFromFuncCall.
  const newInv = {} as Record<string, number>;
  ...
  export async function buildGoalTree(
    ...
    while (frontier.length > 0) {
      ...
      // For non-root nodes, check for direct function call possibility.
      if (currentNode.parentId !== null && currentNode.funcCall === null) {
        ...
        if (!response.toLowerCase().includes("null")) {
          ...
          currentNode.projectedInventory = newInv;
  
          // --- NEW: Bubble the inventory update to the parent node ---
          if (currentNode.parentId !== null) {
            const parentNode = treeNodes.find(
              (n) => n.id === currentNode.parentId
            );
            if (parentNode) {
              parentNode.projectedInventory = newInv;
            }
          }
          // ------------------------------------------------------------
          ...
          // Skip further breakdown for this branch.
          continue;
        }
      }
  
      // Build a structured context from previously processed nodes.
      ...
      // --- SOLUTION 3 CHANGE: Present as JSON ---
      // We'll build a small object summarizing each parent step and its sub-steps:
