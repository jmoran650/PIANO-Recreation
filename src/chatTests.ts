 ...
 */
export async function handleChatTestCommand(agent: AgentBot, username: string, commandMessage: string): Promise<void> {
    ...
    try {
        ...
        console.error(`[${bot.username}] Error executing test command '${commandMessage}':`, error);
        bot.chat(`[${bot.username}] Error running command: ${error.message || String(error)}`);
    }
}

// It might be cleaner to move handleTestGoal here as well,
// or modify handleTestGoal to accept the agent object directly.
// Assuming handleTestGoal remains in testGoal.ts for now but is adjusted
// to potentially accept bot/functionCaller or the full agent object.
// If handleTestGoal needs the full agent, the call above would be:
// await handleTestGoal(agent, ["test", ...parts]);