...
export class Actions {
  ...
  constructor(bot: Bot, navigation: Navigation) {
    this.bot = bot;
  ...
  async attack(mobType: string): Promise<void> {
    ...
    // Find a nearby mob matching the given type.
    const mobs = Object.values(this.bot.entities).filter((entity: any) => {
      return entity.name && entity.name.toLowerCase() === mobType.toLowerCase();
    });
  ...
  async placeCraftingTable(): Promise<void> {
    // Look for the crafting table in the inventory.
    ...
    // Find a safe nearby spot to place the table.
    ...
    try {
      // Look at the center of the safe spot.
      await this.bot.lookAt(safePos.offset(0.5, 0.5, 0.5));
      // Get the block below the safe spot as the placement reference.
  ...
  private findSafePlacement(): Vec3 | null {
    ...
    // Loop horizontal rings from 1 to 3 blocks away.
    for (let d = 1; d <= 3; d++) {
      // For each ring, try two vertical offsets: current level (0) and one above (1)
  ...
  /**
   * Returns all offsets (as Vec3) that form a ring around the origin
   * at the given Chebyshev distance. For example, for distance 1, the ring includes:
   * (1,0), (-1,0), (0,1), (0,-1) and the four diagonals.
   */
 ...
 * This only works if a crafting table is within 3 blocks.
 */
async useCraftingTable(): Promise<void> {
    // Look for a crafting table within a maximum distance of 3 blocks.
    const positions = this.bot.findBlocks({
      point: this.bot.entity.position,
      matching: (block: any) => block && block.name === "crafting_table",
      maxDistance: 4.4, //TODO: Take a longer look at this number. Def results in bot failing to place at maximum dist despite capability to do so.
      count: 1,
    });
    ...
    try {
      // Right-click the crafting table (without moving)
