...
export class FunctionCaller {
  ...
  constructor(
    ...
    // NEW: pass in the observer so we can check for attacks
  ...
  public async callOpenAIWithTools(
    ...
    // Check first if the bot is under attack and, if so, prepend a system alert to the prompt
    ...
    for (let loopCount = 0; loopCount < loopLimit; loopCount++) {
      ...
      if (isUnderAttack) {
        ...
        const systemAlert = `[ALERT] The bot is under attack by "${attackerName}". ${attackMsg}`
        console.log("under attack!");
        // Insert this as a system-level message at the front of the conversation
        allMessages.unshift({ role: "user", content: systemAlert })
      }
      ...
      for (const toolCall of msg.tool_calls) {
        ...
        try {
          ...
          // If we failed to craft because not enough ingredients, see if we can offer info
          if (
            fnName === "craft" &&
